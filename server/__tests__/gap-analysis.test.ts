...
describe('computeRoleGaps', () => {
  it('returns empty when no role categories relevant', () => {
    expect(computeRoleGaps({ java: 5 }, CATEGORIES, [])).toEqual([])
  })

  it('classifies missing when no rated skills in a role category', () => {
  ...
  it('returns empty when all role categories meet threshold', () => {
    const ratings = { java: 4, sql: 4, react: 3 }
    expect(computeRoleGaps(ratings, CATEGORIES, ['backend', 'frontend'], 3)).toEqual([])
  })

  it('returns gaps for categories below threshold', () => {
  ...
  it('marks severity=below when rating is 1.6 to just under threshold', () => {
  ...
  it('marks severity=critical when rating is 1.5 or less', () => {
    const ratings = { java: 1, sql: 2 }
    const gaps = computeRoleGaps(ratings, CATEGORIES, ['backend'], 3)
    expect(gaps[0].severity).toBe('critical')
  })

  it('does not flag category exactly at threshold', () => {
    const ratings = { java: 3, sql: 3 }
    expect(computeRoleGaps(ratings, CATEGORIES, ['backend'], 3)).toEqual([])
  })

  it('ignores skills outside role categories', () => {
  ...
  it('sorts missing first, then ascending rating', () => {
  ...
  it('treats rating 0 same as missing (0 filtered)', () => {
  ...
  it('rounds decimal ratings to one place', () => {
