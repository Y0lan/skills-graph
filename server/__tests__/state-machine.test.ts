...
describe('State Machine', () => {
  ...
  describe('invalid transitions', () => {
    it('postule cannot go directly to embauche', async () => {
      const allowed = getAllowedTransitions('postule')
      expect(allowed).not.toContain('embauche')
    })

    it('postule cannot go directly to entretien_1', async () => {
      const allowed = getAllowedTransitions('postule')
      expect(allowed).not.toContain('entretien_1')
    })

    it('preselectionne cannot go directly to embauche', async () => {
      const allowed = getAllowedTransitions('preselectionne')
      expect(allowed).not.toContain('embauche')
    })

    it('skill_radar_envoye cannot go directly to entretien_1', async () => {
      const allowed = getAllowedTransitions('skill_radar_envoye')
      expect(allowed).not.toContain('entretien_1')
    })

    it('entretien_1 cannot go directly to embauche', async () => {
      const allowed = getAllowedTransitions('entretien_1')
      expect(allowed).not.toContain('embauche')
    })

    it('entretien_2 cannot go backwards to entretien_1', async () => {
  ...
  describe('refuse from every status', () => {
    ...
    for (const statut of allStatuts) {
      if (statut === 'embauche' || statut === 'refuse') {
        it(`${statut} cannot transition to refuse (terminal state)`, async () => {
          expect(getAllowedTransitions(statut)).not.toContain('refuse')
        })
      } else {
        it(`${statut} can transition to refuse`, async () => {
  ...
  describe('isSkipTransition', () => {
    it('preselectionne → entretien_1 is a direct transition, not a skip', async () => {
      expect(isSkipTransition('preselectionne', 'entretien_1')).toBe(false)
    })

    it('entretien_1 → entretien_2 is a direct transition, not a skip', async () => {
      expect(isSkipTransition('entretien_1', 'entretien_2')).toBe(false)
    })

    it('entretien_1 → proposition is a valid skip (skips aboro + entretien_2)', async () => {
      expect(isSkipTransition('entretien_1', 'proposition')).toBe(true)
    })

    it('preselectionne → entretien_1 via skip_radar is a valid skip', async () => {
    ...
    it('skill_radar_complete → aboro skips nothing (entretien_1 is not skippable)', async () => {
      expect(isSkipTransition('skill_radar_complete', 'aboro')).toBe(false)
    })

    it('refuse is never a skip target', async () => {
      expect(isSkipTransition('postule', 'refuse')).toBe(false)
      expect(isSkipTransition('entretien_1', 'refuse')).toBe(false)
    })

    it('backwards transitions are not skips', async () => {
      expect(isSkipTransition('entretien_2', 'postule')).toBe(false)
    })

    it('same-status transition is not a skip', async () => {
      expect(isSkipTransition('postule', 'postule')).toBe(false)
    })

    it('postule → embauche is not a valid skip (non-skippable steps in between)', async () => {
  ...
  describe('getSkippedSteps', () => {
    it('entretien_1 → proposition returns [aboro, entretien_2]', async () => {
      expect(getSkippedSteps('entretien_1', 'proposition')).toEqual(['aboro', 'entretien_2'])
    })

    it('entretien_1 → entretien_2 returns [aboro]', async () => {
      expect(getSkippedSteps('entretien_1', 'entretien_2')).toEqual(['aboro'])
    })

    it('postule → preselectionne returns [] (adjacent steps)', async () => {
      expect(getSkippedSteps('postule', 'preselectionne')).toEqual([])
    })

    it('preselectionne → skill_radar_complete returns [skill_radar_envoye]', async () => {
      expect(getSkippedSteps('preselectionne', 'skill_radar_complete')).toEqual(['skill_radar_envoye'])
    })

    it('postule → embauche returns all intermediate steps', async () => {
    ...
    it('returns empty for same status', async () => {
  ...
  describe('constants', () => {
    it('SKIPPABLE_STEPS contains aboro, entretien_2, skill_radar_envoye', async () => {
    ...
    it('SKIPPABLE_STEPS does not contain non-skippable steps', async () => {
    ...
    it('NOTES_REQUIRED contains refuse and embauche', async () => {
      expect(NOTES_REQUIRED.has('refuse')).toBe(true)
      expect(NOTES_REQUIRED.has('embauche')).toBe(true)
    })

    it('NOTES_REQUIRED does not contain other statuses', async () => {
  ...
  describe('all valid transitions', () => {
    for (const [from, targets] of Object.entries(TRANSITION_MAP)) {
      for (const to of targets) {
        it(`${from} → ${to} is a valid direct transition`, async () => {
