...
historyRouter.get('/:slug', async (req, res) => {
    const slug = req.params.slug as string;
    const db = getDb();
    const rows = await db.prepare('SELECT skill_id, old_level, new_level, changed_at FROM skill_changes WHERE slug = ? ORDER BY changed_at ASC').all(slug) as {
        skill_id: string;
        old_level: number;
        new_level: number;
        changed_at: string;
