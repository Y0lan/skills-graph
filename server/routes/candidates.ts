...
function parsePdfUpload(req: import('express').Request): Promise<ParsedUpload> {
    return new Promise((resolve, reject) => {
        ...
        bb.on('file', (_name: string, stream: Readable, info: {
            mimeType: string;
        }) => {
...
candidatesRouter.get('/', async (_req, res) => {
    ...
    FROM candidates c ORDER BY c.created_at DESC`)
        .all() as (Pick<CandidateRow, 'id' | 'name' | 'role' | 'email' | 'created_by' | 'created_at' | 'expires_at' | 'submitted_at'> & {
        has_report: number;
...
candidatesRouter.post('/', createRateLimit, async (req, res) => {
    ...
    if (roleId) {
        const roleRow = await getRole(roleId);
        if (!roleRow) {
    ...
    const existing = email?.trim()
        ? await getDb().prepare('SELECT id FROM candidates WHERE LOWER(TRIM(email)) = LOWER(TRIM(?)) LIMIT 1').get(email.trim()) as {
            id: string;
        } | undefined
        : undefined;
    if (existing) {
    ...
    `).run(name.trim(), role!.trim(), resolvedRoleId, id);
        ...
        await getDb().prepare('INSERT INTO candidates (id, name, role, role_id, email, created_by) VALUES (?, ?, ?, ?, ?, ?)').run(id, name.trim(), role!.trim(), resolvedRoleId, email?.trim() || null, user.slug);
    }
    ...
    if (resolvedRoleId) {
        const openPostes = await getDb().prepare("SELECT id FROM postes WHERE role_id = ? AND statut = 'ouvert' ORDER BY created_at ASC").all(resolvedRoleId) as {
            id: string;
        }[];
        for (const poste of openPostes) {
            const candidatureId = crypto.randomUUID();
            const inserted = await getDb().prepare<{ id: string }>(`
        INSERT INTO candidatures (id, candidate_id, poste_id, statut, canal)
        VALUES (?, ?, ?, 'postule', 'candidature_directe')
        ON CONFLICT (candidate_id, poste_id) DO NOTHING
        RETURNING id
      `).get(candidatureId, id, poste.id);
            if (inserted) {
                await getDb().prepare(`INSERT INTO candidature_events (candidature_id, type, statut_to, notes, created_by)
          VALUES (?, 'status_change', 'postule', 'Création manuelle', ?)`)
                    .run(inserted.id, user?.slug ?? 'system');
            }
        }
    }
...
candidatesRouter.get('/:id', async (req, res) => {
    const candidate = await getDb().prepare('SELECT * FROM candidates WHERE id = ?').get(req.params.id) as CandidateRow | undefined;
    if (!candidate) {
    ...
    const canRetryExtraction = retryEligible && (await getLatestAsset(candidate.id, 'raw_pdf')) !== null;
...
candidatesRouter.patch('/:id/profile-lock', async (req, res) => {
    ...
    const user = getUser(req);
    const result = await setProfileFieldLock({
    ...
    if (!result.ok) {
        res.status(400).json({ error: result.error ?? 'bad path' });
        return;
    }
    res.json({ ok: true, fieldPath, locked });
});
// Update notes
candidatesRouter.patch('/:id/notes', async (req, res) => {
    const { notes } = req.body;
    const result = await getDb().prepare('UPDATE candidates SET notes = ? WHERE id = ?').run(notes ?? null, req.params.id);
    if (result.changes === 0) {
        res.status(404).json({ error: 'Candidat introuvable' });
        return;
    }
...
candidatesRouter.delete('/:id', async (req, res) => {
    try {
        ...
        const candidate = await db.prepare('SELECT id, name FROM candidates WHERE id = ?').get(req.params.id) as {
            id: string;
            name: string;
        } | undefined;
        if (!candidate) {
        ...
        const docs = await db.prepare('SELECT cd.path FROM candidature_documents cd JOIN candidatures c ON c.id = cd.candidature_id WHERE c.candidate_id = ?').all(req.params.id) as {
            path: string;
        }[];
