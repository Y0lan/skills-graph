...
export function buildCompactSkillSummary(ratings: Record<string, number>, topN = 5, bottomM = 3): string {
    ...
    const parts: string[] = [];
    if (top.length > 0)
        parts.push(`  Forces : ${top.join(', ')}`);
    if (bottom.length > 0)
        parts.push(`  À renforcer : ${bottom.join(', ')}`);
...
chatRouter.post('/', requireAuth, async (req, res) => {
    ...
    for (const m of messages) {
        ...
        if (typeof m.content !== 'string' || m.content.length === 0 || m.content.length > 10000) {
    ...
    // Rate limit check
    const db = getDb();
    const count = await db.prepare("SELECT COUNT(*) as cnt FROM chat_usage WHERE user_id = ? AND used_at > now() - interval '1 day'").get(user.id) as {
        cnt: number;
    };
    ...
    if (Array.isArray(context?.slugs)) {
        ...
        if (context?.slug && typeof context.slug === 'string')
            contextSlugs.push(context.slug);
        if (context?.compareSlug && typeof context.compareSlug === 'string')
            contextSlugs.push(context.compareSlug);
    }
    ...
    if (contextSlugs.length === 0) {
        ...
        // §Item 1.
        const team = await computeTeamAggregate();
        if (team && team.submittedCount > 0) {
            contextBlock = `\n\nContexte global de l'équipe (${team.submittedCount}/${team.teamSize} évaluations) :\n`;
            contextBlock += team.categories.map(c => `- ${c.categoryLabel} : moyenne ${c.teamAvgRank.toFixed(1)}/5`).join('\n');
        ...
        for (const s of contextSlugs) {
            const agg = await computeMemberAggregate(s);
            if (!agg)
                continue;
            contextBlock += `\n\nProfil : ${agg.memberName} (${agg.role})\nCatégories :\n${agg.categories.map(c => `- ${c.categoryLabel} : ${c.avgRank.toFixed(1)}/5 (cible: ${c.targetRank}, écart: ${c.gap > 0 ? `-${c.gap.toFixed(1)}` : 'OK'})`).join('\n')}`;
            // Skill-level detail
            const memberRatings = allRatings[s]?.ratings;
            if (memberRatings) {
                contextBlock += `\nDétail des compétences :\n${buildSkillDetail(memberRatings)}`;
            }
            if (agg.profileSummary)
    ...
    try {
        ...
        await db.prepare('INSERT INTO chat_usage (user_id) VALUES (?)').run(user.id);
...
chatRouter.get('/remaining', requireAuth, async (req, res) => {
    const user = getUser(req);
    const db = getDb();
    const count = await db.prepare("SELECT COUNT(*) as cnt FROM chat_usage WHERE user_id = ? AND used_at > now() - interval '1 day'").get(user.id) as {
        cnt: number;
    ...
    if (count.cnt >= DAILY_LIMIT) {
        const oldest = await db.prepare("SELECT used_at FROM chat_usage WHERE user_id = ? AND used_at > now() - interval '1 day' ORDER BY used_at ASC LIMIT 1").get(user.id) as {
            used_at: string;
        } | undefined;
        if (oldest) {
            // oldest.used_at is UTC, add 24h to get when it expires
            const parsed = new Date(oldest.used_at);
            const usedAt = Number.isNaN(parsed.getTime()) ? new Date(`${oldest.used_at}Z`) : parsed;
            resetsAt = new Date(usedAt.getTime() + 24 * 60 * 60 * 1000).toISOString();
        }
    }
