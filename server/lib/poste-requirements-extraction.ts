...
export async function extractPosteRequirements(params: ExtractPosteRequirementsParams): Promise<ExtractPosteRequirementsResult> {
    ...
    for (const raw of result.input.requirements ?? []) {
        ...
        if (candidate.targetLevel > prior.targetLevel
