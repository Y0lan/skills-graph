 ...
 */
async function extractCategorySkills(cvText: string, category: SkillCategory, client: Anthropic, posteContext: PosteContext | null = null): Promise<CategoryExtraction> {
 ...
 */
export async function extractSkillsFromCv(cvText: string, catalog: SkillCategory[], posteContext: PosteContext | null = null): Promise<ExtractionResult | null> {
    if (cvText.length < 50)
        return null;
    ...
    const results = await Promise.allSettled(catalog.map(category => extractCategorySkills(cvText, category, client, posteContext)));
    ...
    for (const key of Object.keys(valid)) {
        if (mergedReasoning[key])
            reasoning[key] = mergedReasoning[key];
        if (mergedQuestions[key])
            questions[key] = mergedQuestions[key];
    }
