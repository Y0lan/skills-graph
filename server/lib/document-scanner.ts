...
function parseClamAvVersion(raw: string): {
    daemonVersion: string;
    signatureVersion?: string;
    signatureDate?: string;
} {
...
async function scanWithClamAV(filePath: string): Promise<ScanEngineSummary & {
    name: 'ClamAV';
}> {
    try {
...
const VT_POLL_TIMEOUT_MS = 180000;
async function scanWithVirusTotal(filePath: string, filename: string): Promise<ScanEngineSummary & {
    name: 'VirusTotal';
}> {
...
export async function scanDocument(filePath: string, filename: string): Promise<ScanResult> {
    ...
    const availableSummaries = [clamSummary, vtSummary].filter(s => s.available) as Array<ScanEngineSummary & {
        available: true;
        clean: boolean;
    }>;
    ...
    if (clamSummary.available) {
        engines.push('ClamAV');
        if (!clamSummary.clean)
            threats.push(...clamSummary.threats.map(t => `ClamAV: ${t}`));
    }
