...
export async function generateComparisonSummary(
  nameA: string, roleA: string, categoriesA: { label: string; avgRank: number; targetRank: number; gap: number }[],
  nameB: string, roleB: string, categoriesB: { label: string; avgRank: number; targetRank: number; gap: number }[],
): Promise<string | null> {
  const apiKey = process.env.ANTHROPIC_API_KEY
  if (!apiKey) {
