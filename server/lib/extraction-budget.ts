...
export async function getCurrentUsage(userSlug: string): Promise<UsageSnapshot> {
    const day = today();
    const row = await getDb().prepare('SELECT count, tokens_spent FROM extraction_usage WHERE user_slug = ? AND day = ?').get(userSlug, day) as {
        count: number;
        tokens_spent: number;
    } | undefined;
    return {
 ...
 * returns it.
 */
export async function chargeRun(userSlug: string): Promise<UsageSnapshot> {
    const day = today();
    const result = await getDb().prepare(`
      INSERT INTO extraction_usage (user_slug, day, count, tokens_spent)
      VALUES (?, ?, 1, 0)
      ON CONFLICT (user_slug, day) DO UPDATE
        SET count = extraction_usage.count + 1
        WHERE extraction_usage.count < ?
      RETURNING count, tokens_spent
    `).get(userSlug, day, DAILY_RUN_CAP) as {
        count: number;
        tokens_spent: number;
    } | undefined;
    if (!result)
...
export async function recordTokens(userSlug: string, tokens: number): Promise<void> {
    const day = today();
    try {
        await getDb().prepare(`
