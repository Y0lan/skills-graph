 ...
 */
export function computeRoleGaps(ratings: Record<string, number>, categories: SkillCategory[], roleCategoryIds: string[], threshold = 3, maxGaps = 3): RoleGap[] {
    const relevantSet = new Set(roleCategoryIds);
    const gaps: RoleGap[] = [];
    for (const cat of categories) {
        if (!relevantSet.has(cat.id))
            continue;
    ...
    gaps.sort((a, b) => {
        if (a.severity === 'missing' && b.severity !== 'missing')
            return -1;
        if (b.severity === 'missing' && a.severity !== 'missing')
            return 1;
