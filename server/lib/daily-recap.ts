 ...
 */
function noumeaWallClockToUtc(stored: string, opts: { dateAnchor?: 'noon' | 'startOfDay' } = {}): Date | null {
...
function loadStageData(stored: string): Record<string, unknown> {
  try { return JSON.parse(stored) as Record<string, unknown> }
  catch { return {} }
}
 ...
 */
export function buildDailyRecap(now: Date = new Date()): DigestPayload | null {
  const db = getDb()
...
function leadEmails(): string[] {
  ...
  // invalid SQL in SQLite. Bail out cheaply (coderabbit minor).
 ...
 */
export async function sendDailyRecap(opts: { baseUrl: string; now?: Date } = { baseUrl: 'https://radar.sinapse.nc' }): Promise<SendDailyRecapResult> {
  const now = opts.now ?? new Date()
  const payload = buildDailyRecap(now)
  if (!payload) {
