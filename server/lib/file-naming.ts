...
export function buildCanonicalFilename(candidateName: string, originalFilename: string, date: Date = new Date()): string {
...
export function buildTypePrefixedFilename(typePrefix: string, candidateName: string, originalFilename: string, date: Date = new Date()): string {
    const { firstname, lastname } = parseName(candidateName);
    const last = filenamePart(lastname) || 'INCONNU';
    const first = filenamePart(firstname) || 'INCONNU';
    const yyyy = date.getUTCFullYear();
