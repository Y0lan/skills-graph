...
export async function putAsset(params: {
    ...
    const existing = await getDb().prepare('SELECT id, candidate_id, kind, mime, size_bytes, sha256, storage_path, created_at FROM candidate_assets WHERE candidate_id = ? AND kind = ? AND sha256 = ?').get(candidateId, kind, sha256) as {
        id: string;
        candidate_id: string;
        kind: AssetKind;
        mime: string | null;
    ...
    await getDb().prepare('INSERT INTO candidate_assets (id, candidate_id, kind, mime, size_bytes, sha256, storage_path) VALUES (?, ?, ?, ?, ?, ?, ?)').run(id, candidateId, kind, mime, sizeBytes, sha256, storagePath);
    const row = await getDb().prepare('SELECT id, candidate_id, kind, mime, size_bytes, sha256, storage_path, created_at FROM candidate_assets WHERE id = ?').get(id) as {
        id: string;
        candidate_id: string;
        kind: AssetKind;
        mime: string | null;
...
export async function readAssetBuffer(id: string): Promise<Buffer | null> {
...
export async function getLatestAsset(candidateId: string, kind: AssetKind): Promise<AssetRecord | null> {
    const row = await getDb().prepare('SELECT id, candidate_id, kind, mime, size_bytes, sha256, storage_path, created_at FROM candidate_assets WHERE candidate_id = ? AND kind = ? ORDER BY created_at DESC LIMIT 1').get(candidateId, kind) as {
        id: string;
        candidate_id: string;
        kind: AssetKind;
        mime: string | null;
