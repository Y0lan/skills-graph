 ...
 */
export async function calculatePosteCompatibility(candidateRatings: Record<string, number>, posteId: string): Promise<number> {
    const db = getDb();
    ...
    // poste-scoped).
    const posteRow = await db.prepare('SELECT role_id FROM postes WHERE id = ?').get(posteId) as {
        role_id: string;
    } | undefined;
    if (!posteRow)
        return 0;
    const requirements = await db.prepare('SELECT skill_id, target_level, importance FROM poste_skill_requirements WHERE poste_id = ?').all(posteId) as {
        skill_id: string;
        target_level: number;
        importance: string;
    }[];
 ...
 * where weight = 2 for 'requis', weight = 1 for 'apprecie'
 */
function calculateWeightedCompatibility(candidateRatings: Record<string, number>, requirements: {
    skill_id: string;
    target_level: number;
    importance: string;
}[]): number {
    ...
    for (const req of requirements) {
        const targetLevel = Number(req.target_level);
        if (!(targetLevel > 0))
            continue;
        const weight = req.importance === 'requis' ? 2 : 1;
        const candidateLevel = candidateRatings[req.skill_id] ?? 0;
        const contribution = (Math.min(candidateLevel, targetLevel) / targetLevel) * weight;
 ...
 */
async function calculateCategoryAverageCompatibility(candidateRatings: Record<string, number>, posteRoleId: string): Promise<number> {
    const db = getDb();
    // Get the categories for this poste's role
    const roleCats = await db.prepare('SELECT category_id FROM role_categories WHERE role_id = ?').all(posteRoleId) as {
        category_id: string;
    }[];
 ...
 */
export async function calculateEquipeCompatibility(candidateRatings: Record<string, number>, posteRoleId: string): Promise<number> {
    const db = getDb();
    // Get the categories for this poste's role
    const roleCats = await db.prepare('SELECT category_id FROM role_categories WHERE role_id = ?').all(posteRoleId) as {
        category_id: string;
...
export async function getPosteCompatBreakdown(candidateRatings: Record<string, number>, posteId: string): Promise<PosteCompatBreakdown> {
    const db = getDb();
    const posteRow = await db.prepare('SELECT role_id FROM postes WHERE id = ?').get(posteId) as {
        role_id: string;
    } | undefined;
    ...
    if (requirements.length > 0) {
        const skillLabels = new Map((await db.prepare('SELECT id, label FROM skills').all() as {
            id: string;
            label: string;
        }[])
            .map(s => [s.id, s.label]));
        ...
        const rawItems = requirements.map((r) => {
            const targetLevel = Number(r.target_level);
            const weight = r.importance === 'requis' ? 2 : 1;
            const candidateLevel = candidateRatings[r.skill_id] ?? 0;
            const contribution = targetLevel > 0
                ? (Math.min(candidateLevel, targetLevel) / targetLevel) * weight
                : 0;
            if (targetLevel > 0) {
                weightedSum += contribution;
                totalWeight += weight;
    ...
    const categories = await db.prepare('SELECT id, label FROM categories').all() as {
        id: string;
        label: string;
    }[];
...
export async function getEquipeCompatBreakdown(candidateRatings: Record<string, number>, posteRoleId: string): Promise<EquipeCompatBreakdown> {
    const db = getDb();
    const roleCats = await db.prepare('SELECT category_id FROM role_categories WHERE role_id = ?').all(posteRoleId) as {
        category_id: string;
    }[];
 ...
 */
export async function getGapAnalysis(candidateRatings: Record<string, number>, posteRoleId: string): Promise<{
    categoryId: string;
    categoryLabel: string;
    candidateAvg: number;
    teamAvg: number;
 ...
 */
export async function calculateGlobalScore(tauxPoste: number | null, tauxEquipe: number | null, tauxSoft: number | null): Promise<number | null> {
    const db = getDb();
    const weights = await db.prepare('SELECT weight_poste, weight_equipe, weight_soft FROM scoring_weights WHERE id = ?')
        .get('default') as {
        weight_poste: number;
    ...
    return Math.round((tauxPoste ?? 0) * wp + (tauxEquipe ?? 0) * we + tauxSoft * ws);
}
/**
 * Calculate compatibility scores for other postes in the same pole.
 */
export async function calculateMultiPosteCompatibility(candidateRatings: Record<string, number>, currentPosteId: string): Promise<{
    posteId: string;
    posteTitre: string;
    tauxPoste: number;
}[]> {
    const db = getDb();
    const currentPoste = await db.prepare('SELECT pole FROM postes WHERE id = ?')
        .get(currentPosteId) as {
        pole: string;
    } | undefined;
    if (!currentPoste)
        return [];
    const otherPostes = await db.prepare('SELECT id, role_id, titre FROM postes WHERE pole = ? AND id != ?').all(currentPoste.pole, currentPosteId) as {
        id: string;
        role_id: string;
        titre: string;
    }[];
    return Promise.all(otherPostes.map(async p => ({
        posteId: p.id,
        posteTitre: p.titre,
        tauxPoste: await calculatePosteCompatibility(candidateRatings, p.id),
    })));
}
/**
 ...
 */
export async function getBonusSkills(candidateRatings: Record<string, number>, posteRoleId: string): Promise<{
    skillId: string;
    skillLabel: string;
    categoryLabel: string;
    score: number;
    ...
    const allSkills = await db.prepare('SELECT s.id, s.label, s.category_id, c.label as category_label FROM skills s JOIN categories c ON c.id = s.category_id').all() as {
        id: string;
        label: string;
        category_id: string;
    ...
    for (const skill of allSkills) {
        if (roleCatIds.has(skill.category_id))
            continue;
