...
for (const [slug, eval_] of Object.entries(all)) {
    ...
    if (!agg)
        continue;
    const summary = await generateProfileSummary(agg.memberName, agg.role, agg.categories.map(c => ({
    ...
    if (summary) {
        await db.prepare('UPDATE evaluations SET profile_summary = ? WHERE slug = ?').run(summary, slug);
        console.log(`[OK] ${slug}`);
    }
    else {
